...
function createRouter(init) {
  ...
  async function startRedirectNavigation(request, redirect6, isNavigation, _temp2) {
    ...
    pendingNavigationController = null;
    let redirectHistoryAction = replace4 === true || redirect6.response.headers.has("X-Remix-Replace") ? Action.Replace : Action.Push;
    let {
      formMethod,
      formAction,
      formEncType
  ...
  async function queryRoute(request, _temp4) {
    ...
    if (result.loaderData) {
      ...
      if ((_result$activeDeferre = result.activeDeferreds) != null && _result$activeDeferre[match.route.id]) {
        data4[UNSAFE_DEFERRED_SYMBOL] = result.activeDeferreds[match.route.id];
      }
      return data4;
    }
    return void 0;
  ...
  "node_modules/react-router-dom/server.js"(exports) {
    ...
    function StaticRouterProvider2({
      ...
      if (hydrate2 !== false) {
        ...
        let json6 = htmlEscape(JSON.stringify(JSON.stringify(data4)));
...
function convertRoutesToDataRoutes2(routes, mapRouteProperties3, parentPath, manifest) {
  if (parentPath === void 0) {
    parentPath = [];
  }
  if (manifest === void 0) {
  ...
  return routes.map((route, index) => {
    ...
    if (isIndexRoute2(route)) {
      let indexRoute = _extends4({}, route, mapRouteProperties3(route), {
        id
      });
      manifest[id] = indexRoute;
      return indexRoute;
    } else {
      let pathOrLayoutRoute = _extends4({}, route, mapRouteProperties3(route), {
        id,
        children: void 0
      });
      manifest[id] = pathOrLayoutRoute;
      if (route.children) {
        pathOrLayoutRoute.children = convertRoutesToDataRoutes2(route.children, mapRouteProperties3, treePath, manifest);
      }
      return pathOrLayoutRoute;
    }
  });
...
var json3 = function json4(data4, init) {
  if (init === void 0) {
    init = {};
  }
  let responseInit = typeof init === "number" ? {
  ...
  return new Response(JSON.stringify(data4), _extends4({}, responseInit, {
    headers
  }));
};
...
var defer3 = function defer4(data4, init) {
  if (init === void 0) {
    init = {};
  }
  let responseInit = typeof init === "number" ? {
...
var redirect3 = function redirect4(url, init) {
  ...
  let headers = new Headers(responseInit.headers);
  headers.set("Location", url);
  return new Response(null, _extends4({}, responseInit, {
    headers
  }));
};
...
function createRouter2(init) {
  ...
  if (init.mapRouteProperties) {
    ...
    mapRouteProperties3 = defaultMapRouteProperties2;
  }
  let manifest = {};
  let dataRoutes = convertRoutesToDataRoutes2(init.routes, mapRouteProperties3, void 0, manifest);
  let inFlightDataRoutes;
  let basename = init.basename || "/";
  let dataStrategyImpl = init.dataStrategy || defaultDataStrategy2;
  let patchRoutesOnNavigationImpl = init.patchRoutesOnNavigation;
  let future = _extends4({
    v7_fetcherPersist: false,
    v7_normalizeFormMethod: false,
    v7_partialHydration: false,
    v7_prependBasename: false,
  ...
  function updateState(newState, opts) {
    ...
    state = _extends4({}, state, newState);
    let completedFetchers = [];
    let deletedFetchersKeys = [];
    if (future.v7_fetcherPersist) {
      state.fetchers.forEach((fetcher, key) => {
  ...
  async function startNavigation(historyAction, location, opts) {
    ...
    pendingNavigationController = null;
    completeNavigation(location, _extends4({
      matches: updatedMatches || matches
  ...
  async function handleLoaders(request, location, matches, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace4, initialHydration, flushSync, pendingActionResult) {
    ...
    revalidatingFetchers.forEach((rf) => fetchControllers.delete(rf.key));
    let redirect6 = findRedirect2(loaderResults);
    if (redirect6) {
      await startRedirectNavigation(request, redirect6.result, true, {
        replace: replace4
      });
    ...
    redirect6 = findRedirect2(fetcherResults);
    if (redirect6) {
      fetchRedirectIds.add(redirect6.key);
      await startRedirectNavigation(request, redirect6.result, true, {
        replace: replace4
    ...
    if (future.v7_partialHydration && initialHydration && state.errors) {
      errors = _extends4({}, state.errors, errors);
    }
    let updatedFetchers = markFetchRedirectsDone();
    let didAbortFetchLoads = abortStaleFetchLoads(pendingNavigationLoadId);
    let shouldUpdateFetchers = updatedFetchers || didAbortFetchLoads || revalidatingFetchers.length > 0;
  ...
  async function startRedirectNavigation(request, redirect6, isNavigation, _temp2) {
    ...
    pendingNavigationController = null;
    let redirectHistoryAction = replace4 === true || redirect6.response.headers.has("X-Remix-Replace") ? Action2.Replace : Action2.Push;
    let {
      formMethod,
      formAction,
      formEncType
    ...
    let activeSubmission = submission || fetcherSubmission;
    if (redirectPreserveMethodStatusCodes2.has(redirect6.response.status) && activeSubmission && isMutationMethod2(activeSubmission.formMethod)) {
      await startNavigation(redirectHistoryAction, redirectLocation, {
  ...
  async function callDataStrategy(type, state2, request, matchesToLoad, matches, fetcherKey) {
    ...
    let dataResults = {};
    try {
      results = await callDataStrategyImpl2(dataStrategyImpl, type, state2, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties3);
    } catch (e) {
      matchesToLoad.forEach((m) => {
        dataResults[m.route.id] = {
          type: ResultType2.error,
      ...
      try {
        await patchRoutesOnNavigationImpl({
          ...
          patch: (routeId, children) => {
            if (signal.aborted)
              return;
            patchRoutesImpl2(routeId, children, routesToUse, localManifest, mapRouteProperties3);
          }
        });
      } catch (e) {
        return {
  ...
  function _internalSetRoutes(newRoutes) {
    manifest = {};
    inFlightDataRoutes = convertRoutesToDataRoutes2(newRoutes, mapRouteProperties3, void 0, manifest);
  }
  function patchRoutes(routeId, children) {
    let isNonHMR = inFlightDataRoutes == null;
    let routesToUse = inFlightDataRoutes || dataRoutes;
    patchRoutesImpl2(routeId, children, routesToUse, manifest, mapRouteProperties3);
    if (isNonHMR) {
      dataRoutes = [...dataRoutes];
      updateState({});
    }
...
function getMatchesToLoad2(history, state, matches, submission, location, initialHydration, skipActionErrorRevalidation, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, deletedFetchers, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, pendingActionResult) {
  ...
  fetchLoadMatches.forEach((f, key) => {
    ...
    if (fetchRedirectIds.has(key)) {
      ...
      shouldRevalidate = shouldRevalidateLoader2(fetcherMatch, _extends4({
        currentUrl,
        currentParams: state.matches[state.matches.length - 1].params,
        nextUrl,
        nextParams: matches[matches.length - 1].params
...
function patchRoutesImpl2(routeId, children, routesToUse, manifest, mapRouteProperties3) {
  var _childrenToPatch;
  let childrenToPatch;
  if (routeId) {
    let route = manifest[routeId];
  ...
  let newRoutes = convertRoutesToDataRoutes2(uniqueChildren, mapRouteProperties3, [routeId || "_", "patch", String(((_childrenToPatch = childrenToPatch) == null ? void 0 : _childrenToPatch.length) || "0")], manifest);
  childrenToPatch.push(...newRoutes);
}
...
async function loadLazyRouteModule2(route, mapRouteProperties3, manifest) {
  if (!route.lazy) {
    return;
  }
  let lazyRoute = await route.lazy();
  ...
  Object.assign(routeToUpdate, routeUpdates);
  Object.assign(routeToUpdate, _extends4({}, mapRouteProperties3(routeToUpdate), {
    lazy: void 0
  }));
}
...
async function callDataStrategyImpl2(dataStrategyImpl, type, state, request, matchesToLoad, matches, fetcherKey, manifest, mapRouteProperties3, requestContext) {
  let loadRouteDefinitionsPromises = matches.map((m) => m.route.lazy ? loadLazyRouteModule2(m.route, mapRouteProperties3, manifest) : void 0);
  let dsMatches = matches.map((match, i) => {
    let loadRoutePromise = loadRouteDefinitionsPromises[i];
    let shouldLoad = matchesToLoad.some((m) => m.route.id === match.route.id);
    let resolve = async (handlerOverride) => {
...
function stripHashFromPath2(path) {
  ...
  return createPath2(_extends4({}, parsedPath, {
    hash: ""
  }));
}
...
var json5 = (data4, init = {}) => {
  return json3(data4, init);
};
var defer5 = (data4, init = {}) => {
  return defer3(data4, init);
};
...
async function singleFetchActionStrategy(request, matches) {
  ...
  if (isResponse3(result.result) || isRouteErrorResponse2(result.result)) {
    return {
  ...
  return decode(body, {
    plugins: [(type, ...rest) => {
      ...
      if (type === "ErrorResponse") {
        ...
        return {
          value: new ErrorResponseImpl2(status, statusText, data4)
        };
      }
...
var NavLink2 = /* @__PURE__ */ React9.forwardRef(({
  ...
  return /* @__PURE__ */ React9.createElement(React9.Fragment, null, /* @__PURE__ */ React9.createElement(NavLink, _extends5({}, props, prefetchHandlers, {
...
var Link2 = /* @__PURE__ */ React9.forwardRef(({
  ...
  let href = useHref(to);
  let [shouldPrefetch, ref, prefetchHandlers] = usePrefetchBehavior(prefetch, props);
  return /* @__PURE__ */ React9.createElement(React9.Fragment, null, /* @__PURE__ */ React9.createElement(Link, _extends5({}, props, prefetchHandlers, {
    ref: mergeRefs(forwardedRef, ref),
    to,
    "data-discover": getDiscoverAttr(discover, isAbsolute, props.reloadDocument)
  })), shouldPrefetch && !isAbsolute ? /* @__PURE__ */ React9.createElement(PrefetchPageLinks, {
...
var Form2 = /* @__PURE__ */ React9.forwardRef(({
  ...
  let isAbsolute = typeof props.action === "string" && ABSOLUTE_URL_REGEX4.test(props.action);
  return /* @__PURE__ */ React9.createElement(Form, _extends5({}, props, {
    ref: forwardedRef,
    "data-discover": getDiscoverAttr(discover, isAbsolute, props.reloadDocument)
  }));
});
...
function Links() {
  ...
  return /* @__PURE__ */ React9.createElement(React9.Fragment, null, criticalCss ? /* @__PURE__ */ React9.createElement("style", {
    ...
    key,
    link
  }) => isPageLinkDescriptor(link) ? /* @__PURE__ */ React9.createElement(PrefetchPageLinks, _extends5({
    key
  }, link)) : /* @__PURE__ */ React9.createElement("link", _extends5({
    key
  }, link))));
}
function PrefetchPageLinks({
  ...
  return /* @__PURE__ */ React9.createElement(PrefetchPageLinksImpl, _extends5({
    page,
    matches
  }, dataLinkProps));
}
...
function PrefetchPageLinksImpl({
  ...
  return /* @__PURE__ */ React9.createElement(React9.Fragment, null, dataHrefs.map((href) => /* @__PURE__ */ React9.createElement("link", _extends5({
    key: href,
    rel: "prefetch",
    as: "fetch",
    href
  }, linkProps))), moduleHrefs.map((href) => /* @__PURE__ */ React9.createElement("link", _extends5({
    key: href,
    rel: "modulepreload",
    href
    ...
    /* @__PURE__ */ React9.createElement("link", _extends5({
      key
    }, link))
  )));
}
function Meta() {
  ...
  return /* @__PURE__ */ React9.createElement(React9.Fragment, null, meta.flat().map((metaProps) => {
    ...
    if ("tagName" in metaProps) {
      ...
      let Comp = tagName;
      return /* @__PURE__ */ React9.createElement(Comp, _extends5({
    ...
    return /* @__PURE__ */ React9.createElement("meta", _extends5({
...
import(${JSON.stringify(manifest.entry.module)});`;
    return /* @__PURE__ */ React9.createElement(React9.Fragment, null, /* @__PURE__ */ React9.createElement("script", _extends5({}, props, {
      suppressHydrationWarning: true,
      dangerouslySetInnerHTML: createHtml(contextScript),
      type: void 0
    })), /* @__PURE__ */ React9.createElement("script", _extends5({}, props, {
      suppressHydrationWarning: true,
...
function DeferredHydrationScript({
  ...
  return /* @__PURE__ */ React9.createElement(React9.Suspense, {
    fallback: (
      ...
      typeof document === "undefined" && deferredData && dataKey && routeId ? null : /* @__PURE__ */ React9.createElement("script", _extends5({}, scriptProps, {
        async: true,
        suppressHydrationWarning: true,
        dangerouslySetInnerHTML: {
          __html: " "
    ...
    children: (data4) => {
      return /* @__PURE__ */ React9.createElement("script", _extends5({}, scriptProps, {
        async: true,
        suppressHydrationWarning: true,
        dangerouslySetInnerHTML: {
  ...
  }) : /* @__PURE__ */ React9.createElement("script", _extends5({}, scriptProps, {
    async: true,
    suppressHydrationWarning: true,
    dangerouslySetInnerHTML: {
      __html: " "
...
function ScrollRestoration2({
  ...
  return /* @__PURE__ */ React11.createElement("script", _extends5({}, props, {
    suppressHydrationWarning: true,
    dangerouslySetInnerHTML: {
...
@remix-run/react/dist/esm/index.js:
   ...
   *)
*/
//# sourceMappingURL=/build/_shared/chunk-6XXJ6MIM.js.map
